...
function haplotype_step2(command) {
    ...
    var url = php_self;
    new Ajax.Updater($('step1'), url, {method: 'post', postBody: param }, { 
        onComplete: function() {
            $('step1').show();
            document.title = title;
            update_side_menu();
...
function load_metabolites3() {
    $('step3').hide();
    var url = php_self + "?function=step3metabolite&pi=" + phenotype_items_str + "&trait_cmb=" + trait_cmb;
    document.title = 'Loading Step1...';
    new Ajax.Updater($('step3'), url, {
        method: 'get',
        onComplete: function() {
...
function update_phenotype_categories(options) {
    ...
    $A(options).each(
            ...
            if (phenotype_categories.selected) {
                select2_str = phenotype_categories.text;
